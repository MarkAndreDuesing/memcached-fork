...
//Encode Precondition (Arrange):
    ...
    size_t input_item_size = 3000;//(just do broad nondet or fixed value for now)
    ...
    for(unsigned int i = 0; i < MAX_NUMBER_OF_SLAB_CLASSES; i++) {//0-63 range
        ...
        slabclass[i].size = 4000; //initialise with nondet or fixed values for now (properly with slabs_init later) 
    ...
    power_largest = 63;//move to harness (properly with slabs_init later) power_largest goes from 1-63 (as can be seen in slabs_init, 
